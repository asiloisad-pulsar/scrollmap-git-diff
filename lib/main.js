...
module.exports = {
  ...
  provideScrollmap() {
    return {
      ...
      getItems: ({ editor }) => {
        // getDecorations() returns decorations in registration order, not
        // document order, so collect ranges and sort by row before merging.
        const ranges = [];
        for (const decoration of this.getGitDiffDecorations(editor)) {
          ...
          ranges.push({ startRow, endRow, cls: mappedCls });
        }
        ranges.sort((a, b) => a.startRow - b.startRow || a.endRow - b.endRow);
        const items = [];
        let lastItem = null;
        for (const { startRow, endRow, cls: mappedCls } of ranges) {
